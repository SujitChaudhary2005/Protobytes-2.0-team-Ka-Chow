    ...
    <style
      dangerouslySetInnerHTML={{
        __html: Object.entries(
          colorConfig.reduce<Record<string, Record<string, string>>>(
            (acc, [key, itemConfig]) => {
              ...
              if (itemConfig.theme) {
                acc["light"] = {
                  ...acc["light"],
                  [cssVarName]: itemConfig.theme.light,
                }
                acc["dark"] = {
                  ...acc["dark"],
                  [cssVarName]: itemConfig.theme.dark,
                }
              } else if (itemConfig.color) {
                acc["light"] = {
                  ...acc["light"],
                  [cssVarName]: itemConfig.color,
                }
                acc["dark"] = {
                  ...acc["dark"],
                  [cssVarName]: itemConfig.color,
          ...
          .map(
            ([selector, styles]) =>
              `${selector === "light" ? `[data-chart="${id}"]` : `.dark [data-chart="${id}"]`} {\n${Object.entries(styles)
                .map(([k, v]) => `  ${k}: ${v};`)
                .join("\n")}\n}`
          )
          .join("\n"),
...
function getPayloadConfigFromPayload(
  ...
  if (
    key in config ||
    (payloadPayload && key in (payloadPayload as Record<string, unknown>))
  ) {
    configLabelKey = key
  } else {
    // try to match by dataKey in payload
    const dataKey =
      "dataKey" in (payload as Record<string, unknown>)
        ? (payload as Record<string, string>).dataKey
        : undefined
